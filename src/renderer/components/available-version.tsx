  ...
  return (props: T) => {
    for (const variant of variants) {
      try {
        ...
        if (store) {
          Common.logger.debug(
            `[@freelensapp/kubeswift-extension]: Rendering ${resourceName} page with API version ${variant.version}`,
          );
          return <variant.PageComponent {...props} />;
        }
      } catch (error) {
        Common.logger.debug(
          `[@freelensapp/kubeswift-extension]: API version ${variant.version} not available for ${resourceName}: ${error}`,
        );
      }
    }
    ...
    Common.logger.info(
      `[@freelensapp/kubeswift-extension]: ${resourceName} CRD not found in cluster (tried versions: ${triedVersions})`,
    );
